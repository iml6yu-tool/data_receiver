...
{
    public static class DataReceiverOPCUAExtension
    {
        /// <summary>
        /// 注入DataReceiverOPCUA数据接收器
        /// </summary>
        ...
        public static IServiceCollection AddReceiver(this IServiceCollection services, DataReceiverOPCUAOption option, bool isAutoLoadNodeConfig = false, List<NodeItem> nodes = null)

        {
            if (services == null)
                throw new ArgumentNullException(nameof(services));

            services.AddSingleton<DataReceiverOPCUA>(provider =>
            {
                var logFactory = provider.GetService<ILoggerFactory>();
                var log = logFactory.CreateLogger<DataReceiverOPCUA>();
                return new DataReceiverOPCUA(option, log, isAutoLoadNodeConfig, nodes);
            });
            return services;
        }
    }
