...
namespace iml6yu.DataService.Modbus
{
    public static class Extension
    {
        ...
        public static int GetNumberOfPoint(this ModbusReadWriteType readType)
        {
        ...
        public static bool VerifyWriteAddress(this string address, out byte slaveAddress, out ModbusReadWriteType writeType, out ushort bits, ref string message)
        {
            ...
            if (array.Length != 3)
            {
                message = $"node({address}) config error.";
                return false;
            }

            if (!byte.TryParse(array[0], out slaveAddress))
            {
                message = $"node({address}) slaveAddress config error.the first bit must byte type(0~255)";
                return false;
            }

            if (!Enum.TryParse(array[1], out writeType))
            {
                message = $"node({address}) wirteType config error.the second bit must ModbusReadType type ModbusReadWriteType";
                return false;
            }

            if (!ushort.TryParse(array[2], out bits))
            {
                message = $"node({address}) bit config error.the third bit must ushort type";
                return false;
            }
            return true;
        }
